...
(defun lain-done-task (text date time link)
  ...
  (org-narrow-to-subtree)
  (switch-to-buffer (current-buffer))
  (org-log-note-update "DONE" date time (if (string-empty-p link) "DONE" (concat "[[" link "]" "[DONE]]")))
  (message (buffer-name (current-buffer)))
  (save-buffer)
  (org-agenda-write-tmp "/tmp/org/ORG-TASK.html"))
...
(defun lain-itried-task (text date time link)
  ...
  (org-narrow-to-subtree)
  (switch-to-buffer (current-buffer))
  (org-log-note-update "DONE" date time (if (string-empty-p link) "ITRIED" (concat "[[" link "]" "[ITRIED]]")))
  (message (buffer-name (current-buffer)))
  (save-buffer)
  (org-agenda-write-tmp "/tmp/org/ORG-TASK.html"))
...
(defun lain-canceled-task (text date time link)
  ...
  (org-narrow-to-subtree)
  (switch-to-buffer (current-buffer))
  (org-log-note-update "CANCELED" date time (if (string-empty-p link) "CANCELED" (concat "[[" link "]" "[CANCELED]]")))
  (message (buffer-name (current-buffer)))
  (save-buffer)
  (org-agenda-write-tmp "/tmp/org/ORG-TASK.html"))
